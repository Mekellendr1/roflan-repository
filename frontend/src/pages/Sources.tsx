...
import { getNotifications, getSources } from '../api/queries'
import type { NotificationItem, Source } from '../lib/types'
...
export default function Sources() {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getSources(), getNotifications()])
      .then(([s, n]) => {
        if (cancelled) return
        setSources(s)
        setNotifications(n)
  ...
  return (
    <div className="animate-fade-in">
      <TopBar
        ...
        subtitle="Откуда система получает информацию о сотрудниках"
      />
      <div className="p-8">
        {loading ? (
          ...
          <>
            ...
            <div className="mt-8">
              ...
              {notifications.length === 0 ? (
                <p className="text-stone-500 text-sm">Уведомлений пока нет</p>
              ) : (
